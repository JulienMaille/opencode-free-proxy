"""AMD Radeon TokenFactory API key pool with rate-limit-aware rotation.

Reads rc- keys from ``amd-api-keys.txt`` (one per line) and hands them
...
progressively and stops being offered. Mirrors :mod:`nvidia_pool`.
"""
import os
import re
import sys
...
def _find_keys_file() -> Path:
    for d in _CANDIDATES:
        if d and d.is_dir():
            candidate = d / "amd-api-keys.txt"
            if candidate.is_file():
                return candidate
    # Default to the first candidate for the "not found" log message.
    return (_CANDIDATES[0] / "amd-api-keys.txt")


KEYS_FILE = _find_keys_file()

# AMD rc-* keys are a FIXED total length: 51 chars = "rc-" (3) plus a
# 48-char hex body [0-9a-f]. Observed across all valid keys.
AMD_KEY_RE = re.compile(r"^rc-[0-9a-f]{48}$")
AMD_KEY_LEN = 51
...
# AMD Radeon TokenFactory API endpoint + client timeout. Mirrors the NVIDIA
...
AMD_MODELS_URL = AMD_BASE_URL + "/models"
AMD_CHAT_URL = AMD_BASE_URL + "/chat/completions"
AMD_CONNECT_TIMEOUT = 5
AMD_READ_TIMEOUT = 600
AMD_STREAM_READ_TIMEOUT = 600


def _log(*a):
    if not _log_stdout:
        return
    print(f"[{time.strftime('%H:%M:%S')}] [amd] " + " ".join(str(x) for x in a), flush=True)


def _load_keys() -> list[str]:
    """Return non-empty trimmed rc- keys from the keys file (any order)."""
    keys: list[str] = []
    try:
    ...
    for line in raw.splitlines():
        ...
        # strip it so the fixed 51-char check still matches.
        line = line.lstrip("\ufeff").strip()
        # Only genuine keys load: AMD rc- keys are a fixed 51-char total
        # (rc- prefix plus a 48-char hex body).
        if len(line) == AMD_KEY_LEN and AMD_KEY_RE.match(line):
            keys.append(line)
    if not keys:
        _log("No rc- keys found in amd-api-keys.txt")
    else:
        _log(f"Loaded {len(keys)} AMD API keys from {KEYS_FILE.name}")
    return keys


class AMDKeyPool:
    """Sticky-primary pool of AMD TokenFactory keys with 429-aware rotation + cooldown."""
